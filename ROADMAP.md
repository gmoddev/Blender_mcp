...
| 0I | Queue/job/content/selector bounds | Pathological selectors and resource floods fail without blocking Blender | In progress: network deadlines/header/body limits and archive count, member, compressed/expanded byte, compression-ratio, and path-complexity ceilings are enforced; job/content/selector work remains |
...
| 0K | Disposable-profile Blender harness and supported-version matrix | Static scan gaps are reproduced and verified live | In progress: Blender 5.2.1 lifecycle, security-policy, provider, filesystem, network, archive, and credential-extension harnesses pass; other supported versions and platforms remain |
