...
| 0F | Explicitly enabled integrations and bounded network/download/archive helpers | Local-file upload, SSRF, redirect, size, archive, cleanup, and consent tests pass | In progress: all providers remain quarantined; shared purpose-scoped HTTPS/download/archive, provider-job, and single-file GLB content-admission helpers are implemented; provider contracts and integration remain |
...
| 0I | Queue/job/content/selector bounds | Pathological selectors and resource floods fail without blocking Blender | In progress: provider jobs, network responses, archive expansion, regex-free name selectors, and single-file GLB structure/content/result deltas are bounded and live-tested; native-import rollback and provider-specific contracts remain |
...
| 0K | Disposable-profile Blender harness and supported-version matrix | Static scan gaps are reproduced and verified live | In progress: Blender 5.2.1 command/provider lifecycles, security-policy, provider quarantine, filesystem, network, archive, and credential-extension harnesses pass; other supported versions and platforms remain |
