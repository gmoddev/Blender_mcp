...
| 8 | External downloads/archives have incomplete deadlines, byte/member/expansion limits, and cleanup; expensive work can block the main thread. | 0F, 0I | `3d9ee40` (bounded archive/artifact boundary); provider containment `918073f` and `394ca05` | All providers remain denied; shared ZIP tests cover traversal, ambiguous/colliding names, links/special types, encryption, compression methods and ratios, compressed/expanded/member/count/path limits, actual streamed-size enforcement, and cleanup | Blender 5.2.1 passes bounded extraction, cleanup, and traversal denial without outside/residual artifacts; bounded-download and integrated off-main-thread provider validation remain required | **PARTIALLY ADDRESSED / FEATURE QUARANTINED** — shared ZIP extraction and in-process artifact cleanup are bounded; network/download, startup cleanup, content selection, and provider jobs remain open |
