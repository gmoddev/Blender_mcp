...
| 8 | External downloads/archives have incomplete deadlines, byte/member/expansion limits, and cleanup; expensive work can block the main thread. | 0F, 0I | `3d9ee40` (bounded archive/artifact boundary); `2421e4a` (bounded network/download boundary); provider containment `918073f` and `394ca05` | All providers remain denied; shared HTTP tests cover DNS/connect/read/total deadlines, redirects, metadata/framing/type/encoding, declared/streamed bytes, truncation, exclusive artifacts, and cleanup; ZIP tests cover traversal, types, collisions, compression/expansion budgets, streamed-size enforcement, and cleanup | Blender 5.2.1 passes deterministic bounded redirect/download cleanup and bounded extraction/traversal denial; controlled network and integrated off-main-thread provider validation remain required | **PARTIALLY ADDRESSED / FEATURE QUARANTINED** — shared downloads, ZIP extraction, and in-process artifact cleanup are bounded; startup cleanup, content selection, cancellation/reconciliation, and provider jobs remain open |
