...
| 8 | External downloads/archives have incomplete deadlines, byte/member/expansion limits, and cleanup; expensive work can block the main thread. | 0F, 0I | `3d9ee40` (bounded archive/artifact boundary); `2421e4a` (bounded network/download boundary); `f642e77` (bounded provider-job lifecycle); `3e9ebde` (bounded single-file GLB admission); provider containment `918073f` and `394ca05` | All providers remain denied; shared HTTP and ZIP tests cover deadlines, redirects, response/archive budgets, exclusive artifacts, and cleanup; provider-job tests cover bounded workers/jobs/retention/waits, cancellation, main-thread commit admission, cleanup outcomes, and redacted terminal state; GLB tests cover exact file ownership, structure/resource budgets, external-resource denial, digest rebinding, and import-result limits | Blender 5.2.1 passes deterministic bounded redirect/download/extraction, off-main-thread provider preparation and cleanup, and a minimal inspected native GLB import/result-delta canary; controlled network and integrated provider validation remain required | **PARTIALLY ADDRESSED / FEATURE QUARANTINED** — shared downloads, ZIP extraction, artifact cleanup, job scheduling, and GLB admission are bounded; startup reconciliation, importer deadlines/rollback, provider contracts, and handler integration remain open |
