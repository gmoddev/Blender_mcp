...
| 5 | Pending timeouts can execute later; lost responses were blindly replayed; request identity was not end-to-end. | 0A, 0D | `71e3258` (correlation/no-replay half) | Stable request ID, response mismatch, rotation ambiguity, and socket-write ambiguity tests; planned queue/ledger race tests | Required for queued/running/commit-before-response cases and OS fault injection | **PARTIALLY ADDRESSED** — no blind bridge replay, but unsafe queue and no reconciliation ledger remain |
