...
|---|---|---|---|
| CMD-001 | A command cancelled while pending never invokes its callable. | Deterministic queue/timer race test. | Implemented; live pending |
| CMD-002 | Timeout never implies non-execution; the response distinguishes pending timeout from running-after-timeout. | State-machine unit and Blender integration tests. | Implemented; live pending |
| CMD-003 | Command state transitions are atomic, monotonic, and terminal states cannot be overwritten by late workers. | Concurrency tests with controlled events. | Implemented; live pending |
| CMD-004 | Retrying a mutation requires an idempotency key or explicit reconciliation of the earlier request. | Duplicate-request integration tests. | Implemented; reconnect/fault injection pending |
...
| MUT-001 | Mutations serialize and expose `Pending`, `Running`, and a truthful terminal/indeterminate state. | Ordered execution tests. | Implemented on shared queue; live pending |
...
| REC-001 | After disconnect or indeterminate completion, a client can query execution state and reconcile scene state before retrying. | Disconnect/reconnect integration test. | Partial: in-process query implemented; live reconnect and direct callbacks pending |
...
- `blender_mcp/core/thread_safety.py`: the shared queue now tombstones pending timeouts/cancellations,
  retains running-after-timeout outcomes, and deduplicates stable request IDs; live Blender races
  and restart loss still require validation.
