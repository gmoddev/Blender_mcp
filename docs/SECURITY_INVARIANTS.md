...
| AUTH-003 | Credentials are high-entropy, rotatable, revocable, never logged, and stored with user-only access where supported. | Unit tests plus platform storage inspection. | Enforced on Windows x64: packaged WinVault backend and unique user-context write/read/delete pass; other platforms are unsupported pending equivalent artifacts and live proof |
...
| THREAD-001 | Only Blender's main thread may call `bpy`; network and worker threads only enqueue work. | Thread assertions and live Blender tests. | Partial: shared command lifecycle, pre-queue authorization, and provider preparation/cleanup boundaries are main-thread safe; execution-engine and remaining background paths need repository-wide audit |
...
| NETWORK-001 | Every outbound provider request is bound to an immutable purpose: HTTPS-only exact host/port/path policy, all-public bounded DNS answers, pinned and verified peer, reauthorized redirects, verified TLS without ambient key logging, and bounded DNS/connect/read/total time. | URL/DNS/peer/redirect/TLS/deadline unit tests plus controlled-network integration tests. | Enforced in the shared helper and Blender 5.2.1 embedded-Python deterministic harness; providers remain quarantined and controlled-network fixtures remain |
| DOWNLOAD-001 | External response metadata and bytes are bounded before import; output is exclusively created in a request-owned workspace and partial output is removed on failure. | Status/header/framing/type/encoding/declared-size/streamed-size/truncation/cleanup tests. | Enforced in the shared helper and Blender 5.2.1 embedded-Python deterministic harness; provider content selection and job integration remain |
...
| PROVIDER-JOB-001 | External preparation is bounded and off-main-thread; each job preserves request identity, deduplicates exact retries, fails conflicting reuse, commits serially only on Blender's main thread, reports cancellation/retry truthfully, cleans artifacts, and retains no callback/payload/exception data after termination. | Capacity, duplicate/conflict, thread, cancellation, commit, cleanup, shutdown, retention, redaction, and Blender-live tests. | Enforced in the shared provider-job helper and Blender 5.2.1 embedded-Python harness; no provider uses it yet and restart reconciliation remains open |
| PROVIDER-CONTENT-001 | A downloaded provider model reaches native import only as one workspace-owned, digest-bound GLB whose container, JSON graph, buffers, accessors, embedded images, and projected/imported counts stay within explicit budgets and contain no external resource or extension expansion. | Header/chunk/version/JSON/graph/buffer/accessor/image/workspace/link/digest/result-delta/rollback tests plus Blender-live native-import canary. | Partial: admission and exact tracked-datablock rollback are Blender 5.2.1 live-tested; no provider uses them, and a synchronous native call cannot enforce a hard preemptive deadline |
| SELECTOR-001 | Caller-controlled object-name selection never executes regular expressions; only bounded exact, prefix, suffix, and `*`/`?` glob matching may inspect a bounded scene-name set on Blender's main thread. | Mode/grammar/length/wildcard/candidate/evaluation/step-limit tests, legacy-field denials, static regex-removal assertions, and Blender-live latency/mutation canaries. | Enforced in generic and advanced batch handlers; Blender 5.2.1 validates 512-object denial and bounded matching |
...
- The shared provider-content helper accepts only one workspace-owned GLB, rejects external
  resources and extensions, bounds its binary/JSON/graph/accessor/image budgets, and revalidates its
  digest before import. The main-thread commit boundary rolls every new tracked datablock back when
  the operator fails, returns late, or exceeds result limits, and verifies original identity and
  selection state. Blender 5.2.1 passes rollback followed by a clean import. Provider wiring, hard
  native-call isolation, and provider-specific contracts remain prerequisites.
