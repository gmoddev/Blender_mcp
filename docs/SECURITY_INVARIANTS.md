...
- The shared provider-content helper accepts only one workspace-owned GLB, rejects external
  ...
  digest before import. Blender 5.2.1 passes a minimal native-import/result-delta canary. Provider
  wiring, importer deadline/rollback, and provider-specific contracts remain prerequisites.
