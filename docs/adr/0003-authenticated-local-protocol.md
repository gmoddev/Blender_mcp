...
Normal traffic uses version-one length-prefixed envelopes with request/session correlation, an 8
...
every subsequent frame; documentation must not claim per-frame cryptographic integrity. OS-backed
credential storage and durable mutation reconciliation remain later foundations.
